...
def MetadataGenerate(
    ...
    redundant_names = list(
        set(
            [
                item
                for item in IC50_df.index
                if list(IC50_df.index).count(item) > 1
            ]
        )
    )
    retain_idx = []
    ...
    for each_drug in IC50_df.index:
        for each_cellline in IC50_df.columns:
            if (
                str(each_drug) in drug_pubchem_id_set
                and each_cellline in common_cell_lines
            ):
                if (
                    not np.isnan(IC50_df.loc[each_drug, each_cellline])
                    and each_cellline in cellline2cancertype.keys()
                    and cellline2cancertype[each_cellline] in TCGA_label_set
                ):
...
def DataSplit(data_idx, TCGA_label_set, n_splits=5):
    ...
    data_train_idx, data_test_idx = [[] for i in range(n_splits)], [
...
def CalculateGraphFeat(feat_mat, adj_list, israndom=False):
    ...
    if israndom:
        ...
        adj_mat[
            feat_mat.shape[0] :, feat_mat.shape[0] :
        ] = random_adjacency_matrix(Max_atoms - feat_mat.shape[0])
...
def FeatureExtract(
    ...
    for each in common_cell_lines:
        dic_cell_line_feat[each] = (
            pd.read_csv(
                "%s/%s.csv" % (celline_feature_folder, each), index_col=0
            )
            .loc[common_genes]
            .values
        )
    ...
    return (
        drug_feat,
        drug_adj,
        np.array(cell_line_data_feature),
        target,
        cancer_type_list,
    )
...
def ModelTraining(
    ...
    callbacks = [
        ...
        MyCallback(
            ...
            result_file_path=result_file_path_callback,
            patience=20,
        ),
    ]
    model.fit(
...
def ModelEvaluate(
    ...
    data_test_idx_current,
    file_path_pcc_log,
    file_path_spearman_log,
    file_path_rmse_log,
    file_path_csv,
    batch_size=32,
):
    ...
    for each in TCGA_label_set:
        ind = [
            b
            for a, b in zip(cancer_type_test_list, list(range(len(Y_pred))))
            if a == each
        ]
        if len(ind) > 1:
            ...
            cancertype2rmse[each] = mean_squared_error(
                Y_pred[:, 0][ind], Y_val[ind], squared=False
            )
            f_out_rmse.write(
                "%s\t%d\t%.4f\n" % (each, len(ind), cancertype2rmse[each])
            )

            cancertype2spearman[each] = spearmanr(
                Y_pred[:, 0][ind], Y_val[ind]
            )[0]
            f_out_spearman.write(
...
def main():
    ...
    batch_size = 128
    nb_epoch = 500
    ppi_adj_info, drug_feature, data_idx = MetadataGenerate(
        Drug_info_file,
        Cell_line_info_file,
        Drug_feature_file,
        PPI_file,
    ...
    data_train_idx, data_test_idx = DataSplit(
        data_idx, TCGA_label_set, n_splits=1
    )
    ...
    (
        X_train_drug_feat,
        X_train_drug_adj,
        X_train_cellline_feat,
        Y_train,
        cancer_type_train_list,
    ) = FeatureExtract(
        data_train_idx,
        drug_feature,
        celline_feature_folder,
    ...
    X_train_cellline_feat = (
        X_train_cellline_feat - X_train_cellline_feat_mean
    ) / X_train_cellline_feat_std
    X_train = [
        X_train_drug_feat,
        X_train_drug_adj,
        X_train_cellline_feat,
        np.array([ppi_adj for i in range(X_train_drug_feat.shape[0])]),
    ]

    (
        X_test_drug_feat,
        X_test_drug_adj,
        X_test_cellline_feat,
        Y_test,
        cancer_type_test_list,
    ) = FeatureExtract(
        data_test_idx,
    ...
    X_test_cellline_feat = (
        X_test_cellline_feat - X_train_cellline_feat_mean
    ) / X_train_cellline_feat_std
    X_test = [
        X_test_drug_feat,
        X_test_drug_adj,
        X_test_cellline_feat,
        np.array([ppi_adj for i in range(X_test_drug_feat.shape[0])]),
    ]

    test_data = [X_test, Y_test]

    model = KerasMultiSourceDualGCNModel().createMaster(
        X_train[0][0].shape[-1], X_train[2][0].shape[-1], unit_list
    )
