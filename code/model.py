...
class KerasMultiSourceDualGCNModel(object):
    ...
    def createMaster(self, drug_dim, cell_line_dim, units_list):
        ...
        cell_line_feat_input = Input(
            shape=(None, cell_line_dim), name="cell_line_feat_input"
        )
        cell_line_adj_input = Input(
        ...
        # bio-graph GCN
        cell_line_feat_input_high = Dense(32, activation="tanh")(
            cell_line_feat_input
        )
        cell_line_feat_input_high = Dropout(0.1)(cell_line_feat_input_high)
        cell_line_feat_input_high = Dense(128, activation="tanh")(
            cell_line_feat_input_high
        )
        cell_line_feat_input_high = Dropout(0.1)(cell_line_feat_input_high)

        cell_line_GCN = GraphConv(
        ...
        cell_line_GCN = GraphConv(units=256, step_num=1)(
        ...
        x_cell_line = GlobalAveragePooling1D(name="CelllineGraph_out")(
            cell_line_GCN
        )

        x = Concatenate(name="Merge_Drug_Cellline_graphs")(
            [x_cell_line, x_drug]
        )
